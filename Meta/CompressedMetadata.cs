...
namespace ILSpy.Meta
{
	sealed class CompressedMetadata : MetadataBase
	{
		...
		{
			if (tableSource is null || tableDest is null)
			{
				throw new Exception("Invalid table");
			}
			if (tablesStream is null)
			{
				throw new Exception("No tables stream");
			}

			var column = tableSource.TableInfo?.Columns[colIndex] ?? throw new Exception("Invalid column");
		...
		protected override uint BinarySearch(Table tableSource, int keyColIndex, uint key)
		{
			...
			var keyColumn = tableSource.TableInfo?.Columns[keyColIndex] ?? throw new Exception("Invalid column");
